...
export default function HomeClient({ initialUser: _initialUser }: HomeClientProps) {
  ...
  const intent = useMemo(() => detectBuildIntent(typedText), [typedText]);
  const shouldShowNudge =
    mode === "chat" && shouldNudgeToBuilder(intent) && !nudgeDismissed;

  // Reset dismissed flag when textarea transitions empty → non-empty (or
  // empties out). A naive `if (length === 0 && nudgeDismissed)` re-fires
  // setNudgeDismissed on every keystroke once the user has typed past 0,
  // forcing a re-render storm. The ref-based gate fires only on the actual
  // edge, so per-keystroke renders stay cheap.
  const lastEmptyRef = useRef(false);
  useEffect(() => {
    const isEmpty = typedText.trim().length === 0;
    if (isEmpty !== lastEmptyRef.current) {
      lastEmptyRef.current = isEmpty;
      if (isEmpty) setNudgeDismissed(false);
    }
  }, [typedText]);

  const handleOpenInBuilder = () => {
    const url = `/builder?prompt=${encodeURIComponent(typedText.trim())}&mode=plan`;
  ...
  // Prevent flash of unstyled content. By the time HomeClient mounts the
  // user is already authenticated (server component shell + middleware),
  // so we only wait for theme/settings hydration here.
  if (!mounted || !isLoaded) {
    return (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {mode === "chat" ? (
          <>
            ...
            {/* Build-intent nudge */}
            {shouldShowNudge && (
              <div className="mx-4 mb-2 flex items-start gap-3 rounded-lg border border-violet-500/30 bg-gradient-to-br from-violet-500/10 to-fuchsia-500/10 px-3 py-2.5 fade-in">
                <div className="w-7 h-7 rounded-md bg-violet-500/20 flex items-center justify-center shrink-0">
                  <Hammer size={14} className="text-violet-500 dark:text-violet-300" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-light-text dark:text-dark-text leading-tight">
                    Sepertinya kamu mau build aplikasi
                  </p>
                  <p className="text-xs text-light-muted dark:text-dark-muted mt-0.5 leading-snug">
                    Buka di App Builder untuk diskusi rencana, lalu generate kode dengan preview live.
                  </p>
                  <div className="flex items-center gap-2 mt-2">
                    <button
                      type="button"
                      onClick={handleOpenInBuilder}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-violet-500 hover:bg-violet-600 text-white text-xs font-medium transition"
                    >
                      <Hammer size={11} />
                      Buka di Builder
                    </button>
                    <button
                      ...
                      className="text-xs text-light-muted dark:text-dark-muted hover:text-light-text dark:hover:text-dark-text px-2 py-1 transition"
                    >
                      Lanjut chat di sini
                    </button>
                  </div>
                ...
                <button
                  type="button"
                  onClick={handleDismissNudge}
                  className="p-1 rounded text-light-muted dark:text-dark-muted hover:bg-light-hover dark:hover:bg-dark-hover transition shrink-0"
                  aria-label="Tutup"
                >
                  <X size={14} />
                </button>
